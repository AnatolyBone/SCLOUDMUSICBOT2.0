...
async function downloadFromYouTube(searchQuery, metadata = null) {
  ...
  try {
    ...
    // Используем yt-dlp который установлен через pip (не через npm)
    return new Promise((resolve, reject) => {
      const ytdlp = spawn('yt-dlp', [ // Просто 'yt-dlp', не полный путь
      ...
      ytdlp.on('error', (error) => {
        if (error.code === 'ENOENT') {
          // Если yt-dlp не найден, пробуем spotdl
          console.error('[YouTube] yt-dlp не найден, пробую spotdl');
          downloadViaSpotdl(metadata, resolve, reject);
        } else {
          reject(error);
        }
      });
      ...
      ytdlp.stdout.once('data', () => {
        console.log('[YouTube] Поток yt-dlp начался');
        streamStarted = true;
...
// Отдельная функция для spotdl с временным файлом
function downloadViaSpotdl(metadata, resolve, reject) {
  if (!metadata?.originalUrl && !metadata?.id) {
    reject(new Error('Нет данных для spotdl'));
    return;
  }
  ...
  // spotdl НЕ умеет выводить в stdout, поэтому сохраняем во временный файл
  const spotdl = spawn('spotdl', [
    'download',
    spotifyUrl,
    '--format', 'mp3',
    '--output', tempFile, // Сохраняем в файл, НЕ в stdout
    '--threads', '10'
  ]);
  ...
  spotdl.stderr.on('data', (chunk) => {
    ...
    errorData += data;
    // spotdl выводит прогресс в stderr, это нормально
    if (data.includes('%')) {
      console.log('[SpotDL]', data.trim());
    }
  });
  
  spotdl.stdout.on('data', (chunk) => {
    console.log('[SpotDL stdout]:', chunk.toString().trim());
  });
  ...
  spotdl.on('close', (code) => {
    if (code === 0) {
      ...
      if (fs.existsSync(tempFile)) {
        ...
        console.log(`[SpotDL] Файл создан: ${(stats.size / 1024 / 1024).toFixed(2)} MB`);
        
        // Создаем поток из файла
        const stream = fs.createReadStream(tempFile);
        
        // Удаляем файл после окончания стрима
        stream.on('end', () => {
          fs.unlink(tempFile, (err) => {
            if (err) console.error(`Не удалось удалить ${tempFile}:`, err);
            else console.log(`[SpotDL] Временный файл удален`);
          });
        });
        
        stream.on('error', (err) => {
          console.error('[SpotDL] Ошибка чтения файла:', err);
          fs.unlink(tempFile, () => {});
        });
        
        resolve(stream);
      } else {
        reject(new Error('SpotDL не создал файл'));
      }
    } else {
      console.error('[SpotDL] Завершился с ошибкой:', errorData);
