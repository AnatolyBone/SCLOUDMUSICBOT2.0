...
const DEFAULTS = {
  ...
  use_proxy: 'false',         // Использовать прокси для yt-dlp запросов
  proxy_url: process.env.PROXY_URL || '',  // URL прокси (если задан в env — берётся как дефолт)
  support_templates: 'Привет! Напиши, пожалуйста, подробнее, какая именно ошибка возникает (желательно со скриншотом).\n' +
    'Спасибо за обращение! Проблема решена, проверьте работу бота ещё раз.\n' +
    'Данный трек недоступен для скачивания из-за ограничений правообладателя в SoundCloud.\n' +
    'Этот трек защищен DRM-защитой (SoundCloud Go+). Платные премиум-треки зашифрованы и технически не могут быть скачаны ботами. Попробуйте найти обычную версию трека.\n' +
    'Извините за временные неудобства. В данный момент на сервере проводятся технические работы.\n' +
    'Пожалуйста, убедитесь, что вы отправляете корректную ссылку на трек или плейлист.',
};
